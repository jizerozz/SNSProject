...
import style from "@/app/(afterlogin)/_component/nav.module.css";

export default function NavMenu() {
    
    const segment=useSelectedLayoutSegment(); //홈, 메시지 등의 페이지를 지정하여 관리, 수정 
    console.log(segment);
    ...
    return (
      ...
      {me?.id && <li>
        <Link href={`/${me?.id}`}>
          <div className={style.navPill}>
            {segment === me.id ? <div style={{fontWeight: 'bold'}}>
                <svg width={26} viewBox="0 0 24 24" aria-hidden="true"
                    className="r-18jsvk2 r-4qtqp9 r-yyyyoo r-lwhw9o r-dnmrzs r-bnwqim r-1plcrui r-lrvibr r-cnnz9e">
                  <g>
                    <path
